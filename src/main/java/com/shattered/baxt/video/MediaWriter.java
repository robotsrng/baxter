...
implements IMediaWriter
{
	...
	MediaWriter(String url, com.shattered.baxt.video.IMediaReader reader)
	{
			...
			BufferedImage image)
	{
		...
		// resample video picture
		IVideoResampler resampler = null;
		if (picture.getPixelType() != DEFAULT_PIXEL_TYPE){
			// if this stream is not in BGR24, we're going to need to
			// convert it.  The VideoResampler does that for us.
			resampler = IVideoResampler.make(picture.getWidth(), 
					picture.getHeight(), DEFAULT_PIXEL_TYPE,
		...
		try {
			...
			if(resampler != null) {
				...
				if (newPic.getPixelType() != DEFAULT_PIXEL_TYPE) {
					throw new RuntimeException("could not resample video" ) ;
				}

				if (stream.getStreamCoder().encodeVideo(packet, newPic, 0) < 0)
