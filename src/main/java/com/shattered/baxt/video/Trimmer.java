...
public class Trimmer
{
	...
	//THIS will probably need to be changeable in the future but it is the picture type of my phones videos

	private Type pixType = IPixelFormat.Type.YUVJ420P;
	...
	int abitrate ;
	int vbitrate ;
	int vbitratetolerance ;
	int vquality ;
	int vstream ;
	double vscaleFactor ;

	String icontainerFormat ;
	String iacodec ;
	int isampleRate ;
	int ichannels ;
	String rotate = "0" ;
	...
	int setupStreams(FileSpecOptions fileSpec)
	{

		inputURL = fileSpec.getInputURL();
		outputURL = fileSpec.getOutputURL();

		mHasAudio = fileSpec.ismHasAudio();
		mHasVideo = fileSpec.ismHasVideo();

		mRealTimeEncoder = fileSpec.ismRealTimeEncoder();

		acodec = fileSpec.getAcodec();
		vcodec = fileSpec.getVcodec();
		astream = fileSpec.getAstream();
		aquality = fileSpec.getAquality();

		sampleRate = fileSpec.getSampleRate();
		channels = fileSpec.getChannels();
		abitrate = fileSpec.getAbitrate();
		vbitrate = fileSpec.getVbitrate();
		vbitratetolerance = fileSpec.getVbitratetolerance();
		vquality = fileSpec.getVquality();
		vstream = fileSpec.getVstream();
		vscaleFactor = fileSpec.getVscaleFactor();

		icontainerFormat = fileSpec.getIcontainerFormat();
		iacodec = fileSpec.getIacodec();
		isampleRate = fileSpec.getIsampleRate();
		ichannels = fileSpec.getIchannels();

		rotate = fileSpec.getRotation() ;
		trim = fileSpec.getTrim();
		int retval = 0;
		...
		for (int i = 0; i < numStreams; i++)
		{
					...
					&& (vstream == -1 || vstream == i))
			{
				...
				//	mIVideoPictures[i] = IVideoPicture.make(pixType, oWidth, oHeight);
				//	mOVideoPictures[i] = IVideoPicture.make(pixType, oWidth, oHeight);

				mIVideoPictures[i] = IVideoPicture.make(pixType, ic
						.getWidth(), ic.getHeight());
				mOVideoPictures[i] = IVideoPicture.make(pixType, oc
	...
	private void printConverterSpecs() {
		System.out.println(inputURL);
		System.out.println(outputURL);
		System.out.println("");
		System.out.println(mHasAudio);
		System.out.println(mHasVideo);
		System.out.println("");
		System.out.println(mRealTimeEncoder);
		System.out.println("");
		System.out.println(acodec);
		System.out.println(vcodec);
		System.out.println(containerFormat);
		System.out.println(astream);
		System.out.println(aquality);
		System.out.println("");
		System.out.println(sampleRate);
		System.out.println(channels);
		System.out.println(abitrate);
		System.out.println(vbitrate);
		System.out.println(vbitratetolerance);
		System.out.println(vquality);
		System.out.println(vstream);
		System.out.println(vscaleFactor);
		System.out.println("");
		System.out.println(icontainerFormat);
		System.out.println(iacodec);
		System.out.println(isampleRate);
	...
	public IVideoPicture checkRotate(IVideoPicture videoFrame) {
		if( rotate == null || rotate.equals(null) ||rotate.isEmpty() || rotate == ("0")) {	
			return videoFrame;
		}
		BgrConverter convert = new BgrConverter(pixType, videoFrame.getWidth(), videoFrame.getHeight(), 1920, 1080);
		BufferedImage input = convert.toImage(videoFrame);
		BufferedImage outputBuff = VideoRotator.rotateImage(Integer.valueOf(rotate), input);
		IVideoPicture outputVid = convert.toPicture(outputBuff, videoFrame.getTimeStamp()) ;
		return outputVid;
	}
	 ...
	 */
	public void run(FileSpecOptions fileSpec)
	{

		/**
		 * Setup all our input and outputs
		 */
		setupStreams(fileSpec);
