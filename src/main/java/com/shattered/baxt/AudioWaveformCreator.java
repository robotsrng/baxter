...
public class AudioWaveformCreator {
	...
	Color imageBackgroundColor = new Color(20,20,20, 255);
	...
	public static void createWave(String inputPath, String outputPath) throws Exception {
		AudioWaveformCreator awc = null ;
		if(inputPath.endsWith("mp3")) { 
			AudioFormatConverter.convert(inputPath, inputPath.replace("mp3","wav"));
			awc = new AudioWaveformCreator(inputPath.replace("mp3", "wav"), outputPath);
		}else { 
			awc = new AudioWaveformCreator(inputPath, outputPath);
		}
	...
	class SamplingGraph implements Runnable {
		...
		public void saveToFile(String filename) {            
			...
			BufferedImage flippedImg = new BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB); 
			Graphics2D g3 = flippedImg.createGraphics();
			g3.drawImage(bufferedImage, 0, 0, null);
			flippedImg = createFlipped(flippedImg);
			addImage(bufferedImage, flippedImg, 1, 0, 0);
			g3.dispose();
			g2.dispose();
